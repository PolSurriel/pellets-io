...
var ARD = setInterval(function () {
    ...
    colisionReactElements.forEach(function (rElement) {
        colisionableElements.forEach(function (element) {
            if (rElement.colision(element))
                rElement.colisionReact(element);
        });
        enemys.forEach(function (enemy) {
            if (rElement.colision(enemy.ply))
    ...
    pj.cleanShadows();
    shadowElements.forEach(function (sElement) { pj.updateShadow(sElement); });
...
setInterval(function () {
    ...
    dynamicElements.forEach(function (element) { element.draw(ctx); });
    enemys.forEach(function (enemy) { if (pj.iSee(enemy.ply))
        enemy.ply.draw(ctx); });
    staticElements.forEach(function (element) { element.draw(ctx); });
