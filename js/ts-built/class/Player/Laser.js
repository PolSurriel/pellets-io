    ...
    Laser.prototype.colision = function (obj, xp, yp) {
        ...
        for (var i = 0; i < this.linesRemoving.length; i++) {
            if (!this.linesRemoving[i].colision) {
                var r2 = false;
                if (obj instanceof Circle || obj instanceof Player)
                    r2 = PHY.getDistanceBetween(this.linesRemoving[i].xD, this.linesRemoving[i].yD, obj.getX(), obj.getY()) < obj.getRadio();
                else if (obj instanceof Polygon)
                    r2 = PHY.insidepPoly([this.linesRemoving[i].xD, this.linesRemoving[i].yD], obj.getPoints());
                else
                    r2 = this.linesRemoving[i].yD > obj.getMinY() && this.linesRemoving[i].yD < obj.getMaxY() && this.linesRemoving[i].xD > obj.getMinX() && this.linesRemoving[i].xD < obj.getMaxX();
                if (r2) {
